...
public class ModelController{
    ...
    protected boolean freezeStateGod = false;

    protected boolean freezeStateGod1 = false;
    ...
    protected final ArrayList<String> list_nenum = new ArrayList<>();
    ...
    public void onNREG() throws IOException {
        if(this.getClass().getSimpleName().equals("StartController"))
        {
            ...
            User user = startController.dataImpl.getuser(per_reg);
            ...
            startController.neWarn(startController.ernenum);
    ...
    public void onNREP() throws IOException {
        ...
        if(this.getClass().getSimpleName().equals("StartController"))
        {
            ...
            projController.neWarn(projController.ernenum);
            projController.neWarn(projController.gen);
            User user = projController.dataImpl.getuser(Start.usename);
            projController.init();
    ...
    public void add_null(String id)
    {
        list_null.add(id);
    ...
    public void rem_null(String id)
    {
        list_null.remove(id);
        if(list_null.isEmpty()) {
            neWarn(ernull);
            if(!list_nonlat.contains(id) && !list_nenum.contains(id)) redfield((TextField) Start.roots.get(id), 0);
        }
    }

    public void add_nenum(String id)
    {
        list_nenum.add(id);
        redfield((TextField) Start.roots.get(id), 1);
        onWarn(ernenum);
        time_nenum.setText(new SimpleDateFormat("HH:mm").format(new Date()));
    }

    public void rem_nenum(String id)
    {
        list_nenum.remove(id);
        if(list_nenum.isEmpty()) {
            neWarn(ernenum);
            if(!list_nonlat.contains(id) && !list_null.contains(id)) redfield((TextField) Start.roots.get(id), 0);
        }
    }

    public void add_nonlat(String id)
    {
        list_nonlat.add(id);
        redfield((TextField) Start.roots.get(id), 1);
        onWarn(erpat);
    ...
    public void rem_nonlat(String id)
    {
        list_nonlat.remove(id);
        if(list_nonlat.isEmpty()) {
            neWarn(erpat);
            if (!list_null.contains(id) && !list_nenum.contains(id)) redfield((TextField) Start.roots.get(id), 0);
        }
    }
    ...
    public void onPan(MouseEvent mouseEvent)
    {
        Pane but = (Pane) mouseEvent.getSource();
        if(freezeStateGod && !but.getId().equals("pan_zhanr") || but.getId().equals("pan_zhanr") && freezeStateGod1) return;
        ColorAdjust colorAdjust = (ColorAdjust) but.getEffect();
        List<KeyValue> kv = new ArrayList<>();
        kv.add(new KeyValue(colorAdjust.brightnessProperty(), 0, inter));
        played(kv, 100);
    }

    public void nePan(MouseEvent mouseEvent)
    {
        Pane but = (Pane) mouseEvent.getSource();
        if(freezeStateGod && !but.getId().equals("pan_zhanr") || but.getId().equals("pan_zhanr") && freezeStateGod1) return;
        ColorAdjust colorAdjust = (ColorAdjust) but.getEffect();
        List<KeyValue> kv = new ArrayList<>();
        kv.add(new KeyValue(colorAdjust.brightnessProperty(), 0.5, inter));
        played(kv, 100);
    }

    public void onPan1(MouseEvent mouseEvent)
    {
        Pane but = (Pane) mouseEvent.getSource();
        DropShadow dropShadow = (DropShadow) but.getEffect();
        List<KeyValue> kv = new ArrayList<>();
        kv.add(new KeyValue(dropShadow.colorProperty(), Color.web("#ff9500",1), inter));
        played(kv, 100);
    }

    public void nePan1(MouseEvent mouseEvent)
    {
        Pane but = (Pane) mouseEvent.getSource();
        DropShadow dropShadow = (DropShadow) but.getEffect();
        List<KeyValue> kv = new ArrayList<>();
        kv.add(new KeyValue(dropShadow.colorProperty(), Color.web("#ff9500",0), inter));
        played(kv, 100);
    }
